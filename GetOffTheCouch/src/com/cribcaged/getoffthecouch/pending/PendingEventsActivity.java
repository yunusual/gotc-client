...
import com.cribcaged.getoffthecouch.history.EventHistoryDetailsActivity;
...
/**
 * Activity that displays the list of pending events.
 * @author Yunus Evren
 */
public class PendingEventsActivity extends Activity {

	private TableLayout tableLayout;
	private LayoutInflater inflater;

	private LinearLayout loadingLayout;
	private LinearLayout noPendingEventsLayout;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		loadingLayout = (LinearLayout) this.findViewById(R.id.pendingevents_loadinglayout);
		noPendingEventsLayout = (LinearLayout) this.findViewById(R.id.pendingevents_nopendingeventslayout);
		
		rightArrow = (ImageView) this.findViewById(R.id.pendingevents_rightarrowimage);
		rightArrow.setVisibility(View.INVISIBLE);
		rightArrow.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if(selectedEvent!=null){
					Intent intent = new Intent(PendingEventsActivity.this, PendingEventDetailsActivity.class);
		...
		leftArrow = (ImageView) this.findViewById(R.id.pendingevents_leftarrowimage);
		leftArrow.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				PendingEventsActivity.this.finish();
			}
		});
		
		selectedEvent = null;

		tableLayout = (TableLayout) this.findViewById(R.id.pendingevents_tablelayout);
		...
		GetPendingEventsTask getPendingEventsTask = new GetPendingEventsTask();
		getPendingEventsTask.execute();
    }
    
    /**
     * Populates the rows with the details of pending events
     */
    ...
    /**
     * Handles the selection of a pending event
     * @param position - index of the selected pending event
     */
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.activity_pending_events, menu);
        return true;
    }

    /**
     * Asynchronous task that gets the pending events of the user.
     * @author Yunus Evren
     */
    class GetPendingEventsTask extends AsyncTask<Void, Void, Boolean>{

		@Override
		protected void onPreExecute() 
		{
			Log.i(MainActivity.LOG, "GetCompletedEventsTask executed");
			super.onPreExecute();
		}
		...
		protected Boolean doInBackground(Void... unused) {
			if(!NetworkManager.isNetworkAvailable(PendingEventsActivity.this)){
				Log.i(MainActivity.LOG, "GetCompletedEventsTask: Network unavailable");
				return false;
			}
			else{
				try{
					String link = "http://50.7.195.130:8080/gotc/servlet/com.cribcaged.getoffthecouch.server.GetPendingEvents?"
					...
					if(count==0){
						Log.w(MainActivity.LOG, "GetCompletedEventsTask query returned no result.");
						return false;
					}
					...
					e.printStackTrace();
					Log.w(MainActivity.LOG, "GetCompletedEventsTask Null Pointer Exception");
					return false;
				} catch (MalformedURLException e) {
					e.printStackTrace();
					Log.w(MainActivity.LOG, "GetCompletedEventsTask MalformedURLException");
					return false;
				} catch (IOException e) {
					e.printStackTrace();
					Log.w(MainActivity.LOG, "GetCompletedEventsTask IOException");
					return false;
				}
			}
		}
		...
		protected void onPostExecute(Boolean result) {
			if(result){
				Log.w(MainActivity.LOG, "GetCompletedEventsTask finished successfully.");
				if(eventList.size()>0){
					setEventRows();
				}
			}
			else{
				noPendingEventsLayout.setVisibility(View.VISIBLE);
			}
    ...
    /**
     * Asynchronous task that downloads the thumbnail image of an event location.
     * @author Yunus Evren
     */
