...
/**
 * Activity that displays the list of completed events to the user.
 * @author Yunus Evren
 */
public class EventHistoryActivity extends Activity {
	...
	private LinearLayout noCompletedEventsLayout;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		loadingLayout = (LinearLayout) this.findViewById(R.id.eventhistory_loadinglayout);
		noCompletedEventsLayout = (LinearLayout) this.findViewById(R.id.eventhistory_noeventscompletedlayout);
		
		rightArrow = (ImageView) this.findViewById(R.id.eventhistory_rightarrowimage);
		...
		rightArrow.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(selectedEvent!=null){
					Intent intent = new Intent(EventHistoryActivity.this, EventHistoryDetailsActivity.class);
					...
					bundle.putInt("category_id", selectedEvent.getCategoryId());
		...
		leftArrow = (ImageView) this.findViewById(R.id.eventhistory_leftarrowimage);
		...
		tableLayout = (TableLayout) this.findViewById(R.id.eventhistory_tablelayout);
		...
		GetCompletedEventsTask getCompletedEventsTask = new GetCompletedEventsTask();
		getCompletedEventsTask.execute();
    }
    
    /**
     * Populates the rows with each of the completed event's data
     */
    ...
    /**
     * Handles the selection of a completed event
     * @param position - index of the selected completed event
     */
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.activity_event_history, menu);
        return true;
    }
    
    /**
     * Asynchronous task that gets the completed events from the server.
	 * @author Yunus Evren
	 */
    class GetCompletedEventsTask extends AsyncTask<Void, Void, Boolean>{
		...
		protected Boolean doInBackground(Void... unused) {
			if(!NetworkManager.isNetworkAvailable(EventHistoryActivity.this)){
				Log.i(MainActivity.LOG, "GetCompletedEventsTask: Network unavailable");
				return false;
			}
			else{
				try{
					String link = "http://50.7.195.130:8080/gotc/servlet/com.cribcaged.getoffthecouch.server.GetCompletedEvents?"
		...
		protected void onPostExecute(Boolean result) {
			if(result){
				...
				noCompletedEventsLayout.setVisibility(View.VISIBLE);
			}
    ...
    /**
     * Asynchronous task that downloads the thumbnail photo of a completed event's location.
	 * @author Yunus Evren
	 */
